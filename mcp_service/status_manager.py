...
class MCPStatusManager:
    def __init__(self, redis_host: str = 'localhost', redis_port: int = 6379):
        ...
        self.status_key = f'service:{self.service_name}:status'
        self.error_key = f'service:{self.service_name}:error'
        self.last_check_key = f'service:{self.service_name}:last_check'
        self.health_key = f'service:{self.service_name}:health'
        self.start_time_key = f'service:{self.service_name}:start_time'
        
        # Data source keys
        self.data_source_status_key = 'service:data_source:status'
        self.data_source_error_key = 'service:data_source:error'
        self.data_source_last_check_key = 'service:data_source:last_check'
    ...
    def _check_data_source(self):
        ...
        try:
            # Add your data source health check here
            # For example, try to connect to the data source
            # or check if it's responding to requests
            return True
        except Exception as e:
        ...
        try:
            ...
            status = {
                ...
                'components': {
                    ...
                    'data_source': self._check_data_source_status()
                }
            }
            return status
        except Exception as e:
    ...
    def _check_data_source_status(self) -> Dict[str, Any]:
        ...
        try:
            # Add your data source health check logic here
            return {
                'status': 'healthy',
                'message': 'Data source is available'
            }
        except Exception as e:
            return {
                'status': 'error',
    ...
    def _update_redis_status(self, status: Dict[str, Any]):
        ...
        try:
            ...
            self.redis_client.expire('service:status:mcp', 3600)
        except Exception as e:
